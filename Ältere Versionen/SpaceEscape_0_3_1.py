...
def gefangenentraktZentrale(environment):
    ...
    if cmd == 'umsehen':
        ...
        if iMMonsterLP[0] <= 0:
            ...
            time.sleep(5)
            print("Er dreht sich wieder um und schaut wieder die Monitore an.")
            time.sleep(2)
            gefangenentraktZentrale(environment)
...
def maschinenraum(environment):
    if iMMonsterLP[0] >= 0 and mMonsterGesehen[0] == 'Ja':
        ...
        time.sleep(1.5)
        print("Ein riesiges Monster steht zehn Meter von dir entfernt.")
        time.sleep(2)
        ...
        if cmd == '1':
            ...
            time.sleep(0.5)
            print("Du öffnest die Tür zum Generatorraum und tritts ein")
...
def mannschaftsquartiereFlur1(environment):
    ...
    if cmd == '1':
        ...
        time.sleep(0.5)
        print("Du siehst dich um, entdeckst aber nichts interessantes außer den zwei Türen")
        print("in diesem Flur")
        time.sleep(0.5)
        mannschaftsquartiereFlur1(environment)
...
def hauptzentrale(environment):
    ...
    if cmd == '1':
        ...
        print("Sie trägt eine elegante grau-grüne Robe, und sieht dich seelenruhig an.")
...
def waffenkammer(environment):
    if 'Jonas Laserkanone' in mW and iBewFrau[0] == 'Aggressiv':
        ...
        print("Du erzählst ihr, dass du die Laserkanone in einem Schrank im Maschinenraum")
        print("gefunden hast.")
        print("Außerdem erzählst du ihrm wie es dort unten aussah.")
...
def labor(environment):
    if iVerProf[0] == 'Verrückt':
        ...
        if mStoffKuscheltier[0] == 'Rotauge':
            print("Er sieht dich erwartungsvoll an, als er jedoch merkt, wie verwirrt du scheinst")
    ...
    if cmd == '1':
        ...
        print("Unten anhgekommen öffnest du eine weitere Luke und kletterst hinaus.")
        print("Du springst heraus, dabei schließt sich die Luke.")
        print("Du probierst sie zu öffnen, jedoch ohne Erfolg.")
        generatorraum(environment)
...
def medAbteilung(environment):
    if 'Jonas Laserkanone' in mW and iMedAbtEingang[0] == 'Verschlossen':
        ...
        if cmd == '1':
            ...
            mHP.remove(10)
            mHP.append(3)
            print("'Was ist denn mit dir geschehen?!")
            print(" Du siehst aus, als hätte dich ein Raumschiff überfahren!")
            print(" Lass mich kurz deine Wunden versorgen...")
            print("...")
            mLP[0] = mLP[0] - mLP[0] + 100
            print(" Das wars.")
            print(" Falls du hier nochmal mit Kleidung, die besseren Schutz bietet, als die meinigen,")
            print(" vorbei kommst, gebe ich dir etwas dafür. Mein Name ist übrigens Edith.'")
            print("Die alte Dame geht wieder durch die Tür.")
            print("Du folgst ihr")
            medAbteilung(environment)
            
        elif cmd == '2':
            print("'Dann geh doch bitte, wenn du so freundlich wärst.")
            print("Du gehst.")
            medAbteilungFlur(environment)
...
def gefangenentraktZentraleKeinNarrezMehr(environment):
    ...
    if cmd == 'umsehen':
        ...
        if iGenerator[0] == 'An':
            ...
            time.sleep(0.5)
            print("Du steigst hinein, aktivierst sie und wirst in den Weltraum geschossen.")
            print("ENDE")
            exit()
        else:
            time.sleep(0)
...
def endeKeineFluchtkapsel(environment):
    ...
    print("Weder Narrez noch die Fluchtkapsel sind noch da.")
